...
/**
 * Check if a search query is requesting the official servers page
 */
...
/**
 * Parse `server:<id-or-name>` query values
 */
...
export async function renderOfficialServerDetailPage(
  ...
  try {
    ...
    bindOfficialServersHandlers(resultContainer, usernameInput, form);
    updateQueryString(`server=${encodeURIComponent(server._id)}`);
  } catch (error) {
    resultContainer.innerHTML = `
      <section class="official-servers-page">
